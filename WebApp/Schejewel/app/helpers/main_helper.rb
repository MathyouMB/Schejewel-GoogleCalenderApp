module MainHelper
    ...
    CREDENTIALS_PATH = 'C:\sites\Schejewel\app\helpers\credentials.json'.freeze

    # The file token.yaml stores the user's access and refresh tokens, and is
    # created automatically when the authorization flow completes for the first
    # time.

    TOKEN_PATH = 'C:\sites\Schejewel\app\helpers\token.yaml'.freeze
    ...
    #puts parseEmail(event.creator.email)
    #puts event.start.date_time
    #puts parseTimeTimes(event.start.date_time)
    #puts parseTimeTimes(event.end.date_time)
    #puts calcHowManyBlocks(event.start.date_time, event.end.date_time)
    #puts calcHowManyBlocks(DateTime.new(event.start.date_time.year,event.start.date_time.month,event.start.date_time.day,0,0,0),event.start.date_time)
